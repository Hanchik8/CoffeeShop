-- Seed categories
insert into category (name, language, image_filename)
select 'Coffee', 'ru', '1763369959398_photo_2025-11-14_19-49-36.jpg'
where not exists (select 1 from category where name = 'Coffee' and language = 'ru');

insert into category (name, language, image_filename)
select 'Tea', 'ru', '33d00ea9-70c9-468a-84f3-4309ec9c257e.jpg'
where not exists (select 1 from category where name = 'Tea' and language = 'ru');

insert into category (name, language, image_filename)
select 'Desserts', 'ru', '4e8d6851-2990-4d15-820f-4625c2fe0afa.jpg'
where not exists (select 1 from category where name = 'Desserts' and language = 'ru');

insert into category (name, language, image_filename)
select 'Breakfast', 'ru', '64f67683-2b29-44a1-a3c7-bffc88a9a150.jpg'
where not exists (select 1 from category where name = 'Breakfast' and language = 'ru');

insert into category (name, language, image_filename)
select 'Coffee', 'en', '6806705d-3cde-4635-a6d4-16dba5d0725d.jpg'
where not exists (select 1 from category where name = 'Coffee' and language = 'en');

insert into category (name, language, image_filename)
select 'Tea', 'en', '93ff58f0-81e8-4321-9e96-5f6ebbf88385.jpg'
where not exists (select 1 from category where name = 'Tea' and language = 'en');

insert into category (name, language, image_filename)
select 'Desserts', 'en', 'efe479fa-93ef-4168-96b3-3dc5a685be02.jpg'
where not exists (select 1 from category where name = 'Desserts' and language = 'en');

insert into category (name, language, image_filename)
select 'Breakfast', 'en', '33d00ea9-70c9-468a-84f3-4309ec9c257e.jpg'
where not exists (select 1 from category where name = 'Breakfast' and language = 'en');
